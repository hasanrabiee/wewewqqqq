                ...
                <div class="border rounded d-block float-left border"
                    ...
                    <div class="border rounded d-inline float-left scroll_box"
                        ...
                        <form style="height: 7px;margin-bottom: 23px;width: 205px;" method="get"
                              action="{{route('AdminOperator.RegisteredVisitor')}}">
                            <div onblur="is_typing = false" onfocus="is_typing = true" class="form-group"
                                 style="width: 402px;">
                                <input class="form-control float-left" type="search"
                                       placeholder="Registered Visitors List"
                                       style="width: 211px;height: 33px;margin-bottom: 24px;" name="SearchTerm">
                            ...
                            >
                                <a href="?UserID={{$user->id}}">
                                    <p class="text-left d-inline float-left"
                                       style="margin-top: 4px;margin-left: 6px;margin-right: 7px;color: #000000;">
                                    ...
                                    @if($user->AccountStatus == 'Suspend')
                                        <div class="text-right border rounded float-right"
                                             style="height: 29px;width: 89px;margin-top: 4px;margin-right: 14px;padding: 0px;background-color: #ed1c24;">
                                            <p class="d-inline"
                                               style="margin-right: 6px;color: rgb(255,255,255);">{{__('message.Suspended')}}</p>
                                        </div>
                                    @endif
                                </a>
                            </div>
                    ...
                    @if(request()->UserID)
                        <div class="border rounded float-left"
                            ...
                            <div class="border rounded border-dark"
                                 style="height: 270px;width: 527px;margin-left: 7px;margin-top: 9px;padding: 14px;padding-top: 7px;padding-bottom: -1px;">
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.fn')}}:</strong>&nbsp;<span>{{$User->FirstName}}</span></p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.ln')}}:&nbsp;</strong><span>{{$User->LastName}}</span></p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.Gender')}}:&nbsp;</strong><span>{{$User->Gender}}</span></p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.BirthDate')}}:</strong>&nbsp;<span>{{$User->BirthDate}}</span>
                                </p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.Profession')}}
                                        :</strong>&nbsp;<span>{{$User->Profession}}</span></p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.City')}}:</strong>&nbsp;<span>{{$User->City}}</span></p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.Country')}}:&nbsp;</strong><span>{{$User->Country}}</span></p>
                                <p class="text-left" style="color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>{{__('message.email')}}:&nbsp;</strong><span>{{$User->email}}</span></p>
                                <p class="text-left" style=" color: #7b7b7b;margin-bottom: 5px;">
                                    <strong>Feedback:&nbsp;</strong><span
                                        onclick="Swal.fire('{{$User->VisitExperience}}')" style="cursor:pointer;"
                                        class="fa fa-info"></span>
                            </div>
                            <p class="text-left"
                               style="color: #7b7b7b;margin-left: 12px;margin-top: 14px;font-size: 24px;width: 544px;">{{__('message.Payment')}}
                                :
                                @if($User->Payment == 'Paid')
                                    <button class="border rounded"
                                          ...
                                          onclick="areyousure()">{{__('message.Paid')}}</button>
                                @else
                                    <button class="border rounded"
                                          style="margin-left: 30px;background-color: #e91515;color: rgb(255,255,255);padding: 0px;padding-right: 33px;padding-left: 36px;"
                                          onclick="areyousure()">{{__('message.Unpaid')}}</button>
                                @endif
                            </p>
                            <form class="float-left" style="margin-left: 16px;height: 63px;margin-bottom: -25px;"
                                  method="get" action="{{route('AdminOperator.SuspendUser' , $User->id)}}">
                                <div class="form-group text-left" style="margin-bottom: 0px;height: -9px;">
                                    <p class="float-left" style="font-size: 19px;color: rgb(221,20,20);">
                                        ...
                                        <input type="checkbox" style="width: 21px;height: 17px;margin-left: 6px;"
                                               name="AccountStatus"
                                               @if($User->AccountStatus == 'Suspend') checked @endif>
                                    </p>
                                </div>
                                <button class="btn btn-block float-right" type="submit"
                                        style="margin-bottom: 0px;margin-top: -9px;background-color: #05c965;color: rgb(255,255,255);">
                                    {{__('message.Save')}}
                                </button>
                            </form>
                        </div>
                        <form id="mysinsin_form" action="{{route('AdminOperator.UserPaid')}}" method="post">
                            @csrf
